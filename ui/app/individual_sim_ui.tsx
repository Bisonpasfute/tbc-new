...
import { applyIndividualSimSettings, IndividualSimSerializationContext, individualSimSettingsToProto } from '@sim/state/serialization';
...
export class SimHostObject<SpecType extends Spec> implements IndividualSimHost<SpecType> {
	...
	constructor(dom: ShellDom, player: Player<SpecType>, config: SpecDefinition<SpecType>) {
		...
		defaultSimWarnings(this.player).forEach(warning => this.addWarning(warning));
		...
		if (config.reforge) {
			...
			this.reforger = createReforgeOptimizer(this.sim, this.player, {
				...
				// Omitting this leaves the solver un-weighted, and the gem-optimizer/ep-stats probe
	...
	getPresetFilterStorageKey(): string {
		return this.getStorageKey(PRESET_FILTER_STORAGE_KEY);
	}
