 ...
 */
const eligibleBulkSlots = (player: Player<any>, equippedItem: EquippedItem, playerCanDualWield: boolean): BulkSimItemSlot[] => {
...
export const addBulkItems = (player: Player<any>, items: ItemSpec[], silent = false) => {
	...
	items.forEach(item => {
		...
		if (equippedItem) {
			eligibleBulkSlots(player, equippedItem, playerCanDualWield).forEach(bulkSlot => {
