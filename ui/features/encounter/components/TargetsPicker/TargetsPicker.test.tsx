...
const defaultTarget = () => TargetProto.create({ level: 73, swingSpeed: 2 });
...
describe('TargetsPicker', () => {
	...
	it('reads each target row from its own index rather than from the first', () => {
		const encounter = new FakeEncounter([TargetProto.create({ level: 73 }), TargetProto.create({ level: 68 })]);
		mount(encounter);

		const levelOf = (index: number) => (targetRoots()[index].querySelector('#target-' + index + '-picker-level') as HTMLSelectElement).value;
		expect(levelOf(0)).toBe('73');
		expect(levelOf(1)).toBe('68');
	});

	it('writes a level change back to the target it belongs to', () => {
		const encounter = new FakeEncounter([TargetProto.create({ level: 73 }), TargetProto.create({ level: 73 })]);
		mount(encounter);

		const select = targetRoots()[1].querySelector('#target-1-picker-level') as HTMLSelectElement;
		act(() => void fireEvent.change(select, { target: { value: '68' } }));

		expect(encounter.targets.map(target => target.level)).toEqual([73, 68]);
	});
		...
		it('offers one option per npc id, however many presets share it', () => {
			mount(
				new FakeEncounter(
					[defaultTarget()],
					[
						preset('Karazhan/Attumen the Huntsman', 16152),
						preset('Karazhan/Attumen the Huntsman (heroic)', 16152),
						preset('x/Prince Malchezaar', 15690),
					],
				),
			);

			expect(aiOptions()).toEqual(['0', '16152', '15690']);
		});
