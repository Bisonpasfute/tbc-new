...
describe('useSelectorModalState', () => {
	...
	it('leaves the last request in place when closed, the way the vanilla modal kept its contents', () => {
		const { result } = renderHook(() => useSelectorModalState());
		act(() => result.current.openTab(ItemSlot.ItemSlotFeet, SelectorModalTabs.RandomSuffixes, gearData));
		act(() => result.current.setOpen(false));
		expect(result.current.request).toMatchObject({ slot: ItemSlot.ItemSlotFeet, tab: SelectorModalTabs.RandomSuffixes });
	});
});
