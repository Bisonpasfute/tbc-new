...
describe('SlotRail', () => {
	...
	it('renders one icon per slot, in slot order, inside the slots container', () => {
		const { container } = render(<SlotRail gear={gear} currentSlot={null} onOpen={vi.fn()} />);
	...
	it('marks only the icon matching currentSlot as active', () => {
		const { container } = render(<SlotRail gear={gear} currentSlot={ItemSlot.ItemSlotFeet} onOpen={vi.fn()} />);
	...
	it('opens the clicked slot, not the first one', () => {
		const onOpen = vi.fn();
		const { container } = render(<SlotRail gear={gear} currentSlot={null} onOpen={onOpen} />);
