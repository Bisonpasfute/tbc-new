	...
	return (
		<SummaryTableRow>
			<a
				...
				<img
					className="ui-summary-table-gem-icon static inset-gem z-1 inline-block size-gem-inner cursor-pointer rounded-none bg-cover bg-center bg-no-repeat"
...
export const GemSummary = () => {
	...
	const rows = useMemo(() => gemSummaryRows(gear.getAllGems()), [gear]);
		...
		<SummaryTable
			...
			reset={{
				...
				onReset: () => {
					...
					player.setGear(player.getGear().withoutGems());
				},
			}}>
			{rows.map(row => (
